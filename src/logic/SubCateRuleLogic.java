...
import dao.SubCateRuleDao;

public class SubCateRuleLogic{
	...
	public boolean saveRules(AssocRules rules) {
		...
		SubCateRuleDao subCateRule = new SubCateRuleDao();

		if (!subCateRule.deleteAllRules()) {
		    result = false;
		} else {
			...
			if (!subCateRule.saveRule(Integer.parseInt(itemset1.toString()), Integer.parseInt(itemset2.toString()), supportCount, confidence)) {
			    result = false;
			    break;
			}
		    }
